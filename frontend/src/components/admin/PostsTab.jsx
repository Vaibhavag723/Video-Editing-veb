...
export default function PostsTab({ user }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const list = await getAdminPosts(user.id);
  ...
  async function removePost(p) {
    ...
    try {
      await deleteAdminPost(user.id, p.id);
...
function PostEditor({ user, post, onClose }) {
  ...
  async function save() {
    ...
    try {
      if (post) await updateAdminPost(user.id, post.id, form);
      else await createAdminPost(user.id, form);
