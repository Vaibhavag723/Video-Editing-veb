...
const USER_COLS =
  'id, name, email, password_hash, password_plain, is_admin, created_at';
...
async function create(name, email, passwordHash, passwordPlain = '', isAdmin = false) {
  const { rows } = await query(
    `INSERT INTO users (name, email, password_hash, password_plain, is_admin)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id, name, email, is_admin`,
    [name, email, passwordHash, passwordPlain, isAdmin],
  );
  return rows[0];
}

// List every account so the admin panel can view emails, unique IDs, passwords
// and signup dates. Uses a LEFT JOIN to count each user's login events.
async function findAllWithMeta() {
  const { rows } = await query(`
    SELECT u.id, u.name, u.email, u.password_plain, u.is_admin, u.created_at,
...
// Pass null for passwordHash & passwordPlain to keep the current password.
// This never touches the is_admin column — the admin role is immutable.
async function updateUser(id, { name, email, passwordHash = null, passwordPlain = null }) {
  const { rows } = await query(
    `UPDATE users
    ...
    [id, name, email, passwordHash, passwordPlain],
  );
  return rows[0] || null;
}

// Set a brand-new password (used by the "forgot password" reset flow).
async function updatePassword(id, passwordHash, passwordPlain) {
  const { rows } = await query(
    'UPDATE users SET password_hash = $2, password_plain = $3 WHERE id = $1 RETURNING id, name, email, is_admin',
